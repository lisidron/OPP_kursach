...
class AnswerCreator:
    ...
    def create_near_lesson_answer(self, group_data: GroupData, answer: str) -> str:
        """Создает ответ с текущей или ближайшей парой."""
        self.set_today_settings()
        time_now = datetime.now()
        current_time = time_now.hour * 3600 + time_now.minute * 60 + time_now.second
        five_hours_later = current_time + 5 * 3600  # Время через 5 часов

        current_lesson = None
        next_lesson = None

        # Проходим по расписанию, чтобы найти текущую или ближайшую пару
        for day_name, day in group_data.get_days().items():
            for lesson in day.lessons:
                lesson_start_time = lesson.get_start_time_seconds()
                lesson_end_time = lesson.get_end_time_seconds()
                ...
                # Проверяем, является ли пара ближайшей
                if current_time < lesson_start_time <= five_hours_later:
                    if not next_lesson or lesson_start_time < next_lesson.get_start_time_seconds():
                        next_lesson = lesson

            if current_lesson:  # Если текущая пара найдена, дальнейший поиск не требуется
                break

        # Формируем ответ
        if current_lesson:
            answer += (
                f"Сейчас идет пара: {current_lesson.get_subject_type()} {current_lesson.get_name()}\n"
                f"  Преподаватель: {current_lesson.get_teacher()}\n"
                f"  Аудитория: {current_lesson.get_room()}\n"
            )
        elif next_lesson:
            answer += (
                f"Следующая пара: {next_lesson.get_subject_type()} {next_lesson.get_name()}\n"
                f"  Преподаватель: {next_lesson.get_teacher()}\n"
                f"  Время: {next_lesson.get_start_time()} - {next_lesson.get_end_time()}\n"
                f"  Аудитория: {next_lesson.get_room()}\n"
            )
        else:
            answer += "Ближайших пар нет в ближайшие 5 часов.\n"
