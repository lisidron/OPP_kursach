...
class ButtonHandler:
    ...
    async def button_response(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if query.data == "near_lesson":
            ...
            if schedule:
                await query.edit_message_text(text=answerCreator.create_tomorrow_lessons_answer(group_number, get_current_year(), get_current_season()),
            ...
            if schedule:
                await query.edit_message_text(text=answerCreator.create_week_lessons_answer(group_number, get_current_year(), get_current_season()),
